...
/**
 * Taken from [androidx.media3.demo.shortform](https://github.com/androidx/media/blob/51efcad6720d3016d4b17fe8a1e9a11ca89e8b67/demos/shortform/src/main/java/androidx/media3/demo/shortform/PlayerPool.kt)
 */
...
class PlayerPool(private val numberOfPlayers: Int, preloadManagerBuilder: Builder) {
    ...
    private fun acquirePlayerInternal(token: Int, callback: (ExoPlayer) -> Unit) {
        synchronized(playerMap) {
            if (!playerRequestTokenSet.contains(token)) return
    ...
    fun releasePlayer(token: Int, player: Player?) {
        synchronized(playerMap) {
